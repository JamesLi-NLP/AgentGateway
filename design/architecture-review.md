...
>
> **采纳状态（已裁决）：R1~R11 建议 + C3 已全部采纳，作为 D17~D27 并入 design.md 决策记录；A/B/C 类问题对应修订已落地 design.md（见 §8 各条"待修订位置"）。本稿保留作追溯依据；如需归档请另行确认。**
>
> **注**：design.md 已于 D28 后按六域重组章节（§4 总体架构与模块划分 → §5~6 共享基础 → §7~12 逐域详细设计 → §13~16 横切 → §17 决策记录）；本稿所引 design.md § 编号以审阅时版本为准，可经 §17 决策表"修订位置"列换算为新编号。
