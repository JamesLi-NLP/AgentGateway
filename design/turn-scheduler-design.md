...
- 所有异常默认走自动重试（INFRA_RETRY，见 10.2）：已产生确认输出的 Attempt 失败后置 failed，Turn 自动创建新 Attempt 继续；未产生确认输出的故障在同一 Attempt 内重派。
...
| Gateway DB | TurnAdmission（调度队列，同步回执落账）、RuntimeInvocation（含观测水位）、InvocationSpec（含 snapshot_id + content_hash）、LeaseInfo（统一租约，session/controller）、Committed Content（**Gateway 本地镜像/缓存，非权威，5.1.5**） | Gateway |
...
|---|---|---|---|
| **Kubernetes** | 部署形态与副本管理 | Gateway/Runtime 以 Deployment 多副本运行；K8s 负责 Pod 编排、Liveness/Readiness 健康探测、滚动更新与优雅关闭（SIGTERM） | 不参与调度互斥；readiness/liveness 只决定流量分配与滚动更新节奏，不参与任务所有权判定（详见 13.6） |
...
| **Nacos** | 服务注册发现 + 健康感知 + 配置管理 + **观测上报（纯观测）** | Gateway 与 Runtime 注册/摘除/发现（**实例元数据携带 ip:port，调度方直连寻址**，见下方寻址模型）；调度选 Runtime 时过滤不健康实例；运行时配置按精确 Revision 下发；Gateway Pod 将本 Pod InflightRegistry 快照上报 Nacos（8.7），仅用于观测大盘与容量计数 | 心跳感知存在探测窗口（网络分区下可能误判），只能缩短"发现时间"，不能替代租约做"安全接管"判定；**Nacos 上报的值调度决策永不引用**（决策 1）；上报数据取自 Pod 内存（InflightRegistry），不查库 |
...
```text
前端 submitTurn ──> [Gateway 对外 API] ──> [SM] 同步 submit/retry/cancel/update（外部契约 7.1.1）
...
Turn: PENDING -> RUNNING          TurnAdmission: QUEUED -> ACTIVATING
     ...
     / CANCELLED                       -> COMPLETED / REJECTED / FAILED / CANCELLED
                                          |  SUSPENDED（无可用 Runtime 挂起，11.3）
                                          |
Attempt: PENDING -> RUNNING             |
     -> COMPLETED / FAILED        RuntimeInvocation: CREATED -> DISPATCHED
     / CANCELLED                       -> EXECUTING -> COMPLETING
                                          -> COMPLETED / FAILED / CANCELLED
                                          |  PAUSED（未 commit 故障挂起，11.3）
```

两个视角独立演进：**通过同步 submit 回执和 update 回调同步**（外部契约见 7.1.1）；Attempt 终态用 COMPLETED（非 SUCCESS）。

## 4. 总体架构与模块划分（六域）

> **先整体、后逐域**：本章给出模块地图（谁负责什么、边界在哪）；§5~§6 为六域共享的数据/状态/租约基础；§7~§12 按六域逐域展开详细设计；§13~§16 为部署与横切专项。
...
```text
api（对外，用户唯一入口）
    -> TurnProducerService.submitTurn()           // 前端 submitTurn（12.1）
        -> SessionManagementClient.submit()       // 同步调会话管理（7.1.1）
        -> TurnAdmissionRepository.insertReturn() // 回执幂等落账（7.2 步骤 1）
    ...
    -> CancelService.cancelTurn()                 // 用户手动停止（D28）：
                                                  ...
                                                  //   3. RuntimeDispatcher.abort()（携 fencing_token，9.1；排队中跳过）
...
scheduler
    -> TurnScheduler.poll()
        ...
        -> InvocationService.create()
            -> ContextSnapshotClient.store()      // Gateway 构建快照正文 → 会话管理存储（8.4）
...
executor
    -> RuntimeDispatcher.dispatch()               // 从 Nacos 拿 runtime ip:port 直连（9.1）
...
completion
    -> TurnCompletionService.complete()
        ...
        -> SessionManagementClient.update()       // 终态回写（Attempt -> COMPLETED，联动 Turn，10.1）

retry/infra
    -> InfraRetryService
        -> SessionManagementClient.retry(INFRA_RETRY)   // 已 commit 失败后自动新 Attempt（10.2）
        -> RuntimeInvocationRepository.createForRetry()

recovery
    -> RecoveryCoordinator.runRecoveryCycle()   // leaderless|leader 模式开关（11.8）
        -> PodHealthProbe.probe()               // 两段式判死，纯观测（11.6）
        -> ExpiredLeaseRecovery.recover()       // 过期租约回收 + activating 卡死回收（11.5）
        -> StaleInvocationRecovery.recover()    // 卡死判定（判据 2/3）
        -> RuntimeWakeup.recover()              // 唤醒器：可用 Runtime 扫描 + 唤醒 suspended/paused（11.3）
    -> TakeoverExecutor.takeover()              // 安全接管：PG 租约 + epoch 递增 + 新 fencing token（11.2）

streaming
    -> TurnStreamPushService.pushDeltaToStream() // 确认内容/Delta XADD turn_events:<session>（体验层，可丢）
    -> TurnStreamPushService.pushToClient()      // 活跃 attempt 过滤 + SSE 推送 + stream_reset 控制（12.1）
```

### 4.2 六域模块边界与能力（模块划分权威定义）

> 划分原则：**按故障域与职责正交切分**——数据面（streaming，可丢）、控制面（recovery，横切）、权威面（其余四域各管一段状态流转）分离；任何单域崩溃不得传染执行链。"哪个 Gateway Pod 协调某 Turn"**不是任何模块的决策**，由 Phase 2 SKIP LOCKED 竞争涌现（8.1）；模块间只存在唯一的真决策：Runtime 实例选型（executor 域）。
...
| **completion** | 终态判定；update 异步回写；Session 推进；INFRA_RETRY 触发 | TurnCompletionService、InfraRetryService | `runtime_invocation`/`turn_admission` **终态字段**（completed/failed）、`lease_info` release | `markSuccess`/`markFailure`（commit 分界判定，10.2）、`infraRetry`（新 Attempt，复用快照） | 不选型不调度；不持租约续期（续期归持有者 executor）；不直接调 Runtime |
| **recovery** | 接管；判死（纯观测）；卡死回收；挂起唤醒；admin 干预 | RecoveryCoordinator（兼唤醒器 D13）、TakeoverExecutor、PodHealthProbe、ExpiredLeaseRecovery | **跨域抢租约的唯一合法入口**（controller_epoch 递增）；admission suspended↔queued、invocation paused→executing 唤醒 | `takeover`（epoch+新 fencing token）、`runRecoveryCycle`（巡检+唤醒）、`probe`（两段式判死）；admin 四干预（11.7） | 判死≠接管（只缩短时机）；一切干预走状态机合法迁移（5.7）；不参与取消链（取消不是故障） |
...
**对外调用契约（Gateway → 会话管理，完整签名见 7.1.1，决策 D8）：**
...
public interface SessionManagementClient {
    ...
    SnapshotRef storeContextSnapshot(String attemptId, ContextSnapshotBody body); // 快照正文存储（8.4）
}
```
...
public interface InfraRetryService {
    Optional<RuntimeInvocation> infraRetry(RuntimeInvocation failed); // 已 commit 失败 → 新 Attempt（10.2）
}
...
> 五表 DDL 与四状态机是六域共享的权威事实（写权限矩阵见 4.2）；本章由重组前的数据模型、状态机、幂等与一致性三章合并而成。

### 5.1 Gateway 侧（本模块核心）

#### 5.1.1 turn_admission（同步回执落账 + 调度队列，Gateway 侧幂等准入）

Gateway 同步调用会话管理 submit 拿到回执后的准入记录，**承担调度队列**：以回执 `(session_id, turn_id, attempt_id)` 为幂等键 `INSERT ... ON CONFLICT DO NOTHING` 落账，前端重复提交被静默吸收；随后同一行通过状态机流转 `queued → activating → activated → dispatched → …`，被多个 Pod 以 `FOR UPDATE SKIP LOCKED` 竞争消费。无可用 Runtime 时行可置 `suspended` 挂起（唤醒后重新竞争，见 11.3）。
...
suspended（已 commit 失败的 INFRA_RETRY 新 Attempt 在无可用 Runtime 时挂起，11.3）
→ 唤醒后经 queued 重新竞争
```
...
|---|---|---|
| `COORDINATOR_DEAD` | 协调者死亡（判据 1 命中） | 已 commit → Attempt failed + INFRA_RETRY（10.2）；未 commit → 同 Attempt 重派/挂起 |
| `RUNTIME_STALLED` | Runtime 事件停滞（判据 2 命中，5min 阈值） | 同上，按 commit 分界走自动重试 |
| `RETRY_EXCEEDED` | 自动重试超限（重派 N=5 耗尽且 INFRA_RETRY 耗尽/无可用 Runtime） | Turn 置 failed，此后才允许用户重试（10.3） |
...
> 状态 `suspended` 专用于"无可用 Runtime"的挂起（11.3）：已 commit 失败的 Attempt 置 failed 后，其 INFRA_RETRY 新 Attempt 若遇无可用 Runtime，在 admission 层 `suspended` 挂起，不参与竞争；巡检发现可用 Runtime 后按"最新 Turn 的最新 Attempt"优先唤醒置回 queued（决策 D12/D13）。

幂等语义（与 7.3 对应，决策 D15）：

- `(session_id, turn_id, attempt_id)`：同步回执幂等（会话管理返回的 Turn/Attempt 回执唯一；重试产生新 Attempt = 新回执，不会被旧回执吞掉）；
- 用户级防重放（前端重复点击）前移到 Gateway 对外接口 submitTurn 自持 request 幂等键（12.1），**不进调度表**。

#### 5.1.2 runtime_invocation（执行工单 + Runtime 观测）
...
observation 语义：`current_phase / event_sequence / tool_calls_count / output_tokens` 等由 Runtime 事件驱动单调更新，只用于健康感知与观测（见 13.6），不参与状态机判定。

> `paused` 语义（决策 D12）：**未产生确认输出（未 commit）**的 Attempt 遇到无可用 Runtime 故障时，其 Invocation 置 `paused` 挂起——租约仍由持有 Pod 续期（防止其他 Turn 插入同一 Session），Runtime 恢复后由巡检唤醒同 Attempt resume；**一旦曾 commit 过**，则不再置 paused，Attempt 直接走 failed + INFRA_RETRY 新 Attempt（10.2）。

#### 5.1.3 invocation_spec
...
#### 5.1.4 lease_info（统一租约：Session + Controller）

三类租约中，`session_lease`（Session 串行执行权）与 `controller_lease`（Invocation 控制权）结构雷同，合并为一张表以 `lease_type` 区分；Runtime 侧 fencing token 不落库，保持运行时内存语义（见 6.3）。
...
```mermaid
stateDiagram-v2
    [*] --> PENDING: Gateway 同步 submit 创建 Turn + Attempt#1（7.1.1）
    PENDING --> RUNNING: 首次 commit-output/tool-call 隐式触发（5.3）
...
> 系统故障一律 FAILED + fail_reason（D28），Turn 不再有 INTERRUPTED 态；CANCELLED 仅用户取消可达，且仅 USER_RETRY 可恢复（自动重试对 CANCELLED 不生效，10.2）。

### 5.3 Attempt 状态机（会话管理侧）
...
### 5.4 RuntimeInvocation 状态机（Gateway 侧）
...
stateDiagram-v2
    ...
    EXECUTING --> PAUSED: 未 commit 且无可用 Runtime（重派 N=5 耗尽，11.3）
...
> CANCELLED 由用户取消触发（D28）：Gateway 置 admission/invocation 终态后经 **abort 接口**通知 Runtime 停止（9.1）；abort 携带 fencing token、幂等，丢失或超时由 Runtime Execution Lease 到期自停 + fencing 拒收兜底（**abort 是加速器，不是正确性依赖**）。

### 5.5 TurnAdmission 状态机（Gateway 侧）

> 补充自架构审阅 D1：`turn_admission`（5.1.1）是调度队列 + 幂等台账，状态枚举 `queued/activating/activated/dispatched/completed/rejected/failed/cancelled/suspended`，此处补齐 mermaid 图（与 5.3 的 Attempt / 5.4 的 Invocation 三视图并列，各管各的权威域）。

```mermaid
stateDiagram-v2
    [*] --> QUEUED: 回执幂等落账入队（5.1.1）
    QUEUED --> ACTIVATING: Phase 2 SKIP LOCKED 竞争成功
    ACTIVATING --> ACTIVATED: CAS 确认持有 Session 执行权（8.1）
    ACTIVATED --> DISPATCHED: 创建 Invocation 并派发 Runtime（Phase 5）
    DISPATCHED --> COMPLETED: Phase 7 终态（update 异步回写，10.1）
    ...
    QUEUED --> SUSPENDED: 无可用 Runtime（INFRA_RETRY 新 Attempt，11.3）
    SUSPENDED --> QUEUED: 巡检唤醒（可用 Runtime，重新竞争）
    ACTIVATING --> QUEUED: 巡检判据 3（activating 卡死回收，11.5）
    QUEUED --> REJECTED: 准入拒绝（校验不通过）
```

> 用户取消可将**任意非终态** admission 行终态化为 cancelled（D28，图中画三条主路径；ACTIVATING/ACTIVATED 瞬态窗口内取消由 CAS 回收/下拨前检查兜底）；cancelled 行与 failed 行同样走终态归档（14.8）。
...
| TurnAdmission | Gateway（调度权威） | `turn_admission.status` | 调度队列态，含 `suspended` 挂起（11.3） |
| RuntimeInvocation | Gateway（运行权威，可重建） | `runtime_invocation.status` | 运行态，含 `paused` 挂起（11.3） |

> 两套权威各管其域：会话管理管"业务真相"（这个 Turn 该不该重试、哪个 Attempt 是正式回答）；Gateway 管"调度真相"（这个 Attempt 现在排到哪、由谁在执行）。同步提交通道与 update 回调负责两者对齐（7.1.1）。
...
| Turn FAILED | retry(USER_RETRY) | 用户点击（10.3） | PENDING（新 Attempt） | 会话管理 |
| Turn FAILED | retry(INFRA_RETRY) | 已 commit 故障自动重试（10.2） | PENDING（新 Attempt，复用快照） | 会话管理 |
| Turn PENDING / RUNNING | cancel | 用户手动停止（10.2，D28） | CANCELLED（**活跃 Attempt 同事务置 CANCELLED**） | 会话管理 |
| Turn CANCELLED | retryTurn（USER_RETRY） | 用户重试（10.2，D28） | PENDING（新 Attempt） | 会话管理 |
| Invocation DISPATCHED / EXECUTING / PAUSED | abort（用户取消） | cancelTurn 触发（9.1，D28） | CANCELLED | Gateway |
...
| Invocation PAUSED | 巡检唤醒（可用 Runtime） | 唤醒器扫描（11.3） | EXECUTING（同 Attempt resume） | Gateway |
| Admission QUEUED | 新 Attempt 遇无可用 Runtime | INFRA_RETRY 且 Runtime 池空（11.3） | SUSPENDED | Gateway |
| Admission SUSPENDED | 巡检唤醒（可用 Runtime） | 唤醒器扫描（11.3） | QUEUED（重新竞争） | Gateway |
...
> 租约是六域共用的安全基础设施：executor 域持有并续期（心跳），recovery 域接管（epoch 递增）；两权分置见 4.2 跨域不变量 3。

> 物理落点：Session 与 Controller 两类租约都存于 **`lease_info`（5.1.4）**，以 `lease_type` 区分；Runtime 侧 fencing token 为运行时内存语义，不落库。

### 6.1 Session Execution Lease（`lease_info`，lease_type='session'）
...
### 6.2 Controller Lease（`lease_info`，lease_type='controller'）
...
### 6.3 Runtime Execution Lease（Fencing Token）
...
> **生命周期定义（决策 D23，修订自架构审阅 B2）**：fencing token 有效期 = **Invocation 生命周期**（`dispatched` 到终态），**不按固定 lease_duration 过期**——单次 LLM 调用可能超过心跳间隔，固定时长过期会造成慢执行中途失效。代际号 = `controller_epoch`：每次接管（11.2）递增，旧 token 即刻失效。签发时机：Phase 5 dispatch（9.1）与每次重派（重派 = 新 holder、新 token）；校验时机：Runtime 每次写 Sandbox / 调 Tool / commit 前校验；失效语义：Runtime 停止 Agent Loop 与 Tool Call，Sandbox 拒绝写入，commit 被 Gateway fencing 校验拒绝（9.2，决策 D18）。
...
> 本域职责与边界见 4.2（用户唯一入口终接、同步提交通道编排）。本章收拢：会话管理侧契约（7.1）、提交落账流程（7.2）、幂等准入规则（7.3~7.5）。
...
#### 7.1.1 同步提交通道与外部契约（替代原 turn_ready_outbox）
...
前端 submitTurn
  ...
  -> Gateway 以回执幂等落账 turn_admission（5.1.1），进入调度队列（Phase 2~7）
  -> 终态经 update 接口回写会话管理（10.1）
```
...
| **cancel** | `POST /v1/turn/{turn_id}/cancel` | GW → SM | 用户手动停止；**事务内同落 Turn=CANCELLED + 活跃 Attempt=CANCELLED（决策 D28）**，停止自动重试，仅用户可恢复（10.2）；Attempt 的 CANCELLED 永不经 update 写 |
| **commit** | `POST /v1/attempt/{attempt_id}/commit` | GW → SM | 提交确认输出（按 message_id 幂等 upsert 消息表）；**首次 commit 隐式推进 Attempt → RUNNING**（5.3）；请求携带 `attempt_id + controller_epoch`（fencing），旧 Attempt / 非当前 holder 的写入被拒绝（决策 D18，9.2）；**Attempt 已 CANCELLED 时拒收（迟到的取消后写入）** |
| **update** | `POST /v1/turn/{turn_id}/update` | GW → SM | 提交终态（Attempt COMPLETED / FAILED → 联动 Turn），幂等；**枚举仅系统终态两种（决策 D28），用户取消经 cancel 落账** |
| **context-snapshot** | `PUT/GET /v1/context/snapshot/{attempt_id}` | GW ↔ SM | 快照正文存会话管理（权威），Gateway 只留 snapshot_id + content_hash（8.4） |

三段映射（前端 API → Gateway → 会话管理契约）：

| 前端（12.1） | Gateway 动作 | 会话管理契约 |
|---|---|---|
...
**重试模型（决策 D9~D12）**：见 10.2——所有异常默认自动重试；已产生确认输出的 Attempt 失败后置 failed，Turn 自动 INFRA_RETRY 新 Attempt；未产生确认输出的故障在同一 Attempt 内重派（N=5 退避）。

### 7.2 Phase 1: 同步 submit 与幂等准入
...
前端 submitTurn(session_id, input, attachment_ids, selected_skill_ids)
  -> Gateway 校验 + 用户级防重放（submitTurn 自持 request 幂等键，Redis 快速路径可选，12.1）
  -> 调会话管理 POST /v1/turn/submit（同步，7.1.1）
  -> 会话管理事务内创建 Turn + Attempt#1（PENDING），返回回执 { turn_id, attempt_id, session_id }
```
...
| 调度队列 | `turn_admission`（Gateway 库，5.1.1） | 队列 + 幂等台账：`on conflict (session_id, turn_id, attempt_id) do nothing` 吸收重复提交 |
...
> **终态拒绝（决策 D26，修订自架构审阅 R11）**：`turn_admission` 终态行归档/清理后，重放防护**不得依赖行存在性**（`on conflict` 只对活表行生效）——落账前必须做**终态检查**：回执对应的 `(session_id, turn_id)` 若已有终态记录（活表或归档表）则直接返回该 Turn 的终态结果，**不再插入新 queued 行**。这是"重放防护上移"：防线从"行存在性"变为"终态拒绝"，与 A1 的终态异步化共用"对齐检查"路径。数据留存活期见 14.8。
...
| lease_info | `uk(lease_type, session_id)` / `uk(lease_type, invocation_id)` | Session 串行执行权 / Controller 控制权各一份（6.3） |
...
> 本域职责与边界见 4.2（排队竞争激活、冻结、创建 Invocation 与双层租约）。本章收拢：Phase 2~4（8.1~8.3）、快照与 Spec 冻结（8.4~8.5）、调度循环（8.6）、容量门控（8.7）。

### 8.1 Phase 2: 排队与串行激活（分布式竞争）
...
> 实现选型（决策 13）：**不引入 Spring Integration**。`JdbcPollingChannelAdapter` 无法表达"跨库两段语义 + 租约检查 + 容量门控"的组合（三表跨库、多态租约、空闲跳过），硬凑会退化回定制代码；保持 `@Scheduled` + `JdbcTemplate` 手写 poll 循环（8.6），单纯简单、可测、可加门控。

### 8.2 Phase 3: Context 与 Spec 冻结
...
### 8.3 Phase 4: Invocation 创建与租约获取
...
3. Gateway 将二者写入 `invocation_spec`（5.1.3）；Runtime 执行时按 `snapshot_id` 从会话管理取正文（不落 Gateway 库）。
...
> **孤儿快照回收（决策 D24，修订自架构审阅 B3）**：Phase 3 快照正文已存入会话管理，但若随后本地事务（Phase 4）回滚/失败，该 `snapshot_id` 无 Invocation 引用——会话管理侧形成**孤儿快照**（正文仍占用存储）。回收策略：**Attempt 终态时显式回收**——会话管理在 Attempt 进入终态（COMPLETED/FAILED/CANCELLED）时，对该 Attempt 的 `snapshot_id` 做引用计数减一，计数归零即删除正文；Gateway 侧 `invocation_spec` 引用随行归档（14.8）不产生孤儿。孤儿兜底：会话管理侧按"快照创建时间 + 无活跃 Attempt 引用"周期扫描清理（TTL 保留 24h 后删除）。

### 8.5 InvocationSpec 冻结
...
### 8.7 InflightRegistry：Pod 本地执行注册表（决策 1 / 10）
...
> 本域职责与边界见 4.2（Runtime 选型下拨、abort、心跳续期、确认内容幂等落库）。本章收拢：Phase 5 下拨/重派/abort（9.1）、Phase 6 确认累积（9.2）、Delta 模型（9.3）、事件拉模型（9.4）、Sandbox（9.5~9.7）、执行循环（9.8）。

### 9.1 Phase 5: Runtime 调度与执行

事务提交后（Lease 已持有），调用 Runtime：

```text
0. 容量门控：running_count（InflightRegistry 中未过期 lease 条目数，内存计数）已达本 Pod
   max_running（Nacos 动态配置，默认 30~50）时，本 Pod 暂停 claim 并上报 oversubscription（8.7）
...
下拨失败 / Runtime 失联时的自动重试（决策 6 / D9~D12）：
  ...
  - 5 次仍失败且无可用 Runtime：Invocation 置 paused 挂起（11.3），巡检唤醒后 resume；
...
POST /v1/invocation/{invocation_id}/abort        # Runtime 侧提供（Runtime driver 职责"响应取消"的接口化）
  ...
  兜底（abort 是加速器，不是正确性依赖）：
    ...
    迟到的 commit/事件 -> Attempt 已 CANCELLED，会话管理终态守卫拒收（7.1.1）；
    fencing token 失配的残留写入 -> Sandbox/执行端 Capability 校验拒绝（6.3）
```
...
> - **epoch 校验**：`controller_epoch` 必须匹配 lease_info 中当前 Controller Lease holder（11.2 接管递增）；**接管后旧 Worker 的 commit 被 fencing 隔离**，不污染 committed_content 镜像。
> - 被拒写入丢弃并计指标（fencing 冲突计数，见 15.1），不参与状态机。
...
故障时只丢失未确认的 Delta；已确认内容权威在会话管理消息表，Gateway 侧 `committed_content` 仅本地镜像/缓存（决策 D2），接管时装载镜像续读（11.2），可随时丢弃重建。

### 9.3 Delta 与 Committed Content
...
### 9.4 多 Pod 流式链路（Redis Stream 作为事件总线）

> **事件获取：拉模型（决策 D22，修订自架构审阅 B1）**：事件由 **Gateway 主动向 Runtime 拉取**（或长连接持续拉取），而非 Runtime 反向推送 SSE——拉模型的持有方天然跟着 holder 走：接管（11.2）后新 Pod 直接向同一 Runtime 建立新拉取连接，**无需 Runtime 端"事件该推给谁"的寻址协议**。拉取端点 = dispatch 时记录的 `runtime_replica_id` 的 Nacos 实例元数据 ip:port（决策 D14），复用 Invocation 装载后的同一连接。
...
Gateway 持有 Pod（holder）--主动拉取--> Runtime（Agent Loop 事件流）
   ...
   按客户端水位消费              （接管/重连时也能从 Stream 续读）
       |
       v
   用户 session 级 SSE（推送内容见 12.1）
```

关键点：

- **不要用 Redis Pub/Sub 广播**：Pub/Sub 不持久，Pod 重启或断连窗口期事件丢失；Stream 天然支持按 ID 断点续读（`XRANGE turn_events:7 <watermark> +`）。
- **拉模型与接管（11.2 联动）**：旧 holder Pod 断连后，接管 Pod 直接向同一 Runtime 重建拉取连接；若 Runtime 已不可达，按 10.2 的 commit 分界走自动重试。**不存在"Runtime 事件推给旧 Pod 后丢失"的寻址窗口**——拉取方决定连接，跟 holder 走。
...
> 本域职责与边界见 4.2（终态判定、update 异步回写、Session 推进、INFRA_RETRY 触发）。本章收拢：Phase 7 完成与推进（10.1）、自动重试模型（10.2）、幂等重试与配额（10.3）。

### 10.1 Phase 7: 完成与 Session 推进

> **终态回写异步化（决策 D17，修订自架构审阅 A1）**：`update`（远程 RPC）**不放在本地事务内**——本地事务只落终态 + 释放租约，COMMIT 后再异步调会话管理 `update`。消灭"RPC 夹在事务中"的两类窗口：RPC 超时拖住事务行锁、以及 update 已生效但本地 COMMIT 失败导致的两库永久分歧。终态未送达由巡检判据 4 补发（11.5）。
...
--    （POST /v1/turn/{turn_id}/update，幂等；外部契约见 7.1.1，决策 D8）
--    失败/未送达 -> 巡检判据 4（11.5）补发，直至会话管理对齐
```

### 10.2 自动重试模型（决策 D9~D12，取代原"v1 一律中断"语义）
...
异常发生（Runtime 宕机 / 事件停滞 / 连接失败）
  ...
  │            （唤醒对象 = 最新 Turn 的最新 Attempt，11.3）
  └─ 已 commit（有已提交数据）
       └─ 该 Attempt 判 failed（终态溯源 fail_reason 见 5.1.1），已确认内容保留
           └─ Turn 自动 INFRA_RETRY：调 retry(trigger_type=INFRA_RETRY)
               ...
               → 若无可用 Runtime：新 Attempt 在 turn_admission 置 suspended 挂起（11.3）
```
...
| 停止时机 | 停止后 | 恢复方式 |
|---|---|---|
| 无可用 Runtime | attempt 挂起（invocation `paused` 或 admission `suspended`） | RecoveryCoordinator 巡检发现可用 Runtime 后，**逐个唤醒最新 Turn 的最新 Attempt**（11.3） |
...
- 排队或 Runtime 无槽：基础设施重试，不增加 Attempt；Runtime 长期不可用 → 挂起等待巡检唤醒（11.3）。
...
> - USER_RETRY 配额单独维护（用户主动 `retryTurn`，10.3）——**系统性基础设施故障即使触发满额 INFRA_RETRY，也不消耗用户配额**；
> - 杜绝"基础设施抖动耗尽用户重试机会"的体验事故：用户在一次与自己无关的故障后，仍保有完整的手动重试机会。

## 11. recovery 域详细设计

> 本域职责与边界见 4.2（接管、判死纯观测、卡死回收、挂起唤醒、admin 干预）。协调者异常 + 自愈停摆在此兜底；运行期 Runtime 故障由 executor/completion 在主流程自愈（9.1/10.2）。

### 11.1 故障矩阵总览

**故障处理矩阵（总览）**——先看全景，再按 §11.2 / §10.2 细读：


| # | 故障场景 | 检测方式 | 处理策略 | 恢复保证 |
|---|---|---|---|---|
| F1 | Gateway Pod 宕机 / 失联 | Session/Controller Lease 心跳超时；两段式判死（11.6） | PG 弱一致接管：递增 `controller_epoch` + 新 fencing token 后接管（11.2）；旧 holder 被 fencing 拒绝 | 接管后继续调度，Session 单活，Turn 不丢 |
| F2 | Runtime 崩溃 / 失联（未 commit） | Controller Lease 心跳缺失；事件停滞（判据 2/3，11.5） | 同 Attempt 重派 N=5 退避（10.2）；耗尽且无可用 Runtime → invocation `paused` | 调度层自动续跑同 Attempt，用户无感 |
| F3 | Runtime 故障（已 commit） | 事件停滞 / 终态异常 | 已 commit 部分保留；Attempt → FAILED，Turn → INFRA_RETRY 新 Attempt（10.2） | 已确认内容不丢，自动续答（新 Attempt） |
| F4 | 无可用 Runtime | **任务级证据（决策 D27，修订自架构审阅 C3）**：Nacos 健康列表为空 + 该任务连续 N 次派发失败（下拨重试 9.1 耗尽），两条件同时成立——**不单凭 Nacos 瞬时视图**，避免网络分区误挂起健康任务 | 未 commit：invocation `paused`；已 commit 新 Attempt：admission `suspended`（11.3） | 唤醒器巡检自动恢复最新 Turn 最新 Attempt |
| F5 | 用户手动停止 | `cancelTurn` 对外 API | SM.cancel 事务内 Turn+Attempt 同落 CANCELLED（D28）；Gateway 联动 admission/invocation 置 cancelled + Runtime abort（9.1）；停止一切自动重试（10.2） | 仅 USER_RETRY 可重新发起 |
| F6 | 会话管理不可用 / 5xx | 同步 submit/update 超时或异常（7.1.1） | submit 失败直接回 5xx 给前端（7.3）；update 失败重试 | 前端可见失败，幂等重放安全 |
| F7 | 多 Pod 调度竞争 | PG 唯一约束 / SKIP LOCKED 冲突 | Session 串行执行权 + Lease 版本校验；输者放弃（8.1/13.1） | 每个 Turn 恰好被一个 Pod 调度 |
| F8 | 重派 / 重试超限 | 重派 N=5 耗尽；INFRA_RETRY 超限 | 自动重试降级为"等待用户"：Turn → FAILED，仅 USER_RETRY 可恢复（10.3） | 不无限重试，交由用户决策 |
...
4. 按 runtime_replica_id 查 Nacos 实例元数据拿 ip:port，重建 Runtime 连接并重新拉取事件流（拉模型 9.4——拉取方跟 holder 走，无需 Runtime 端寻址；如果 Runtime 仍在运行；决策 D14）
5. 继续从 committed_content 水位恢复（镜像装载到新 Pod）

接管后若 Runtime 已不可恢复（连接失败 / 事件停滞），按 10.2 的 commit 分界走自动重试（未 commit 重派/挂起；已 commit → Attempt failed + INFRA_RETRY）。
```
...
执行形态（决策 7）：v1 采用**全员巡检**——每个 Pod 周期运行 `RecoveryCoordinator`（11.8），用 SKIP LOCKED 竞争接管名额，无单点、无 watch 依赖；`PodHealthProbe`（11.6）与 `TakeoverExecutor` 完全解耦，探测结论只决定"何时尝试接管"的时机，接管安全性仍由下方 SQL 的租约 + epoch 保证。
...
### 11.3 无可用 Runtime 的挂起与唤醒（决策 D12/D13）

巡检（RecoveryCoordinator，11.8）兼任"唤醒器"（不新增组件）：
...
挂起期间 Session/Controller Lease 仍由持有 Pod 续期（防止其他 Turn 插入同一 Session）；若持有 Pod 同时死亡，走 11.2 接管流程，接管者继续承担挂起与后续唤醒。
...
- 用户手动停止：SM.cancel 事务内同落 Turn=CANCELLED + 活跃 Attempt=CANCELLED（D28），自动重试停止，仅用户可再次发起（USER_RETRY）；Gateway 侧异步收敛（admission/invocation 置 cancelled + Runtime abort，9.1）。
...
- 判据 1 命中：走 Session Lease 接管流程（见 11.2），接管者递增 `controller_epoch` 并刷新 fencing token。
- 判据 2 命中：按 commit 分界走 10.2 自动重试——未 commit：同 Attempt 重派/挂起（paused）；已 commit：Attempt 判 failed，Turn 自动 INFRA_RETRY 新 Attempt（已确认内容保留在会话管理 + committed_content 镜像）。
...
- 判据 4 命中（决策 D17）：异步补发会话管理 `update`（幂等，7.1.1），直至对齐确认（`sync_alignment_state` 清除该行）；RPC  transient 失败按退避重试，**不改状态机、不触碰租约**——纯对齐补偿动作。
- 判据 5 命中（决策 D19）：按 commit 分界走 10.2 自动重试（与判据 2 同路径）——覆盖"无限推事件但永不产终态"的死循环 Agent Loop，杜绝资源白占与 Session 串行被卡死。

### 11.6 Pod 死亡判定（两段式，决策 8 / 9）

**红线：判死 ≠ 接管。** 判死是观测结论（记日志 + 指标），接管必须重新走 PG 租约 + `controller_epoch` 递增 + 新 fencing token（11.2）——探测永不直接授权接管。
...
> 修订自架构审阅 A4：§1.1 目标承诺"调度状态可查询、可审计、可人工干预"，此处落地**管理员干预路径**，全部落 `admission_audit` 审计轨迹（干预者、动作、时间、对象、原因），与查询层（12.1）同权分离（admin 而非用户 API）。

| 接口 | 动作 | 目标状态 | 前置条件 | 说明 |
|---|---|---|---|---|
| `adminAdjustPriority` | 调整 `activation_priority` | 调度序次变化（Phase 2 按 priority 竞争） | 目标行未到终态 | 满足 §1.1"可人工干预"与 §16 全局公平 |
| `adminForceFail` | 强制置 failed | admission → failed；invocation → failed | 目标行非终态 | 对"卡死不收敛"任务人工兜底；走 10.2 的 commit 分界语义（已 commit 内容保留） |
| `adminWakeup` | 唤醒挂起任务 | suspended → queued / paused → executing | 目标行当前 suspended/paused | 提前唤醒（不等巡检节奏 11.3） |
| `adminRequene` | 手动重排 | 任意非终态 → queued | 目标行非终态 | 干预异常入队（配合巡检判据 3 的人工版） |

- 与用户 API 隔离：以上仅 admin 权限（RBAC），用户侧对已终态 Turn 只能 `retryTurn` / `cancelTurn`（12.1）；
- 审计落点：`admission_audit(id, session_id, turn_id, actor, action, target_status, reason, created_at)`——**不可变、可追溯**，与终态行同转归档（14.8）；
- 干预与状态机的接缝：forceFail/wakeup 均须走状态机合法迁移（5.7 表内已允许的转移），不直接 UPDATE 绕过约束。

### 11.8 恢复执行者抽象：RecoveryCoordinator（决策 7）
...
public interface RecoveryCoordinator {
    ...
    void runRecoveryCycle();                      // 全员巡检：回收过期租约 + 卡死判定（11.5）
...
- 两模式共享同一接管 SQL 与 fencing 协议（11.2），切换只影响"谁执行巡检"，不影响安全性。

**巡检兼任唤醒器（决策 D13）**：`RuntimeWakeup.recover()` 在巡检循环内执行——维护可用 Runtime 池（Nacos 健康实例 + readiness，元数据 ip:port），扫描发现可用后按"最新 Turn 的最新 Attempt"唤醒挂起任务（admission `suspended` → queued；invocation `paused` → resume），见 11.3。唤醒不引入新组件，复用全员巡检的 SKIP LOCKED 竞争。

## 12. streaming 域详细设计

> 本域职责与边界见 4.2（Delta 体验层，可丢、永不写 PG 权威状态）。本章收拢：前端流式接口与 Attempt 屏蔽（12.1）。

### 12.1 前端屏蔽 Attempt：推送契约
...
| `cancelTurn` | 用户手动停止（调用 cancel，自动重试停止，仅用户可恢复，10.2） | 屏蔽 |
...
#### 13.6.2 Gateway 探针接口（K8s 使用，非调度使用）
...
>
> 单 Pod 并发由 `turn-scheduler.max-running-turns.per-pod`（默认 30~50，Nacos 动态配置热生效）门控；15~25 Pod 是"每 Pod 跑满 max_running"的期望形态，实际 Pod 数随容量门控与负载自动伸缩（8.7）。
...
监控先行：以上阈值依赖 14.2 的指标持续观测，**不要靠预估扩容，靠监控数据触发**。
...
| 专项 | 场景 | 预期行为 |
|---|---|---|
| 同秒双 Pod 接管 | 两个 Gateway Pod 同秒发现同一 Session lease 过期、同时 SKIP LOCKED 竞争 | 恰好一个成功（epoch CAS 胜出），另一个 conflict 计数 +1（15.1） |
| SKIP LOCKED 并发竞争 | N 个 Pod 同时 poll 同一批 queued admission 行 | 行级互斥：无重复激活，无死锁（锁等待超时可控） |
| RPC 超时注入 | submit/update 通道注入 5s/30s/∞ 超时 | submit：回执幂等重试可重入；update：异步化后本地事务不受拖累（10.1），巡检判据 4 补发 |
| commit 与接管竞态 | Runtime 慢 commit 期间 lease 被接管 | fencing 双侧校验拒绝旧 holder 写入（D18）；新 holder 重新执行 |
| 终态未对齐 | update 永久失败（网络分区） | 判据 4 持续重试直至对齐；状态分歧计数可观测（15.1） |

> **数据库迁移策略（修订自架构审阅 D3）**：全部 DDL（表/索引/归档操作）纳入**版本化迁移工具**（Flyway/Liquibase），随应用发布；归档 Job、历史表迁移脚本与常规迁移共用版本序列，保证 `turn_admission` 冷热分层（14.8）等结构变更可回滚、可审计。

### 14.8 终态数据生命周期与归档（决策 D26/D27）

> 修订自架构审阅 C1/R11：Turn 终态后五表数据不能一刀切"清删"或"全留"，按职责分层——**台账冷热分层，执行事实转历史，镜像可删，租约自清理**。

| 表 | 职责 | 终态后处理 | 保留周期（建议） |
|---|---|---|---|
| `turn_admission` | 调度队列 + **幂等台账**（7.3 重放吸收） | **冷热分层，不物理删除**：终态行转入归档区；活表只留未终态 + 重放窗口内终态行 | 活表保留 30 天（覆盖重放窗口），到期转归档，归档长期保留 |
...
**归档前置条件（重放防护上移，决策 D26）**：归档/删除 admission 行前，防重放必须从"行存在性"升级为"**终态拒绝**"——落账前查回执对应 `(session_id, turn_id)` 是否已有终态（活表或归档表），有则直接返回终态结果、不再插入 queued 行（见 7.3）。此检查与 A1 的终态对齐共用路径，未落地前不得启用归档。
...
- 查询层（12.1 getTurn / 管理查询）按需 union 活表 + 归档表；
...
| invocation_paused_total | Invocation paused 挂起数（无可用 Runtime，11.3） |
| admission_suspended_total | admission suspended 挂起数（11.3） |
| runtime_wakeup_total | 唤醒器唤醒次数（11.3，按 suspended/paused 分） |
...
3. ✅ 已拍板（D9~D12 取代）：failed 为终态前先走自动重试——未 commit 同 Attempt 重派 N=5，已 commit Attempt 判 failed 后 Turn 自动 INFRA_RETRY，超限才置 failed（fail_reason 枚举见 5.1.1）。
4. ✅ 已拍板（D9~D12 取代）：重派 N=5 + 退避 1s→2s→4s→8s→16s 仅用于未 commit 阶段；已 commit 阶段由 INFRA_RETRY 新 Attempt 接管（10.2）。
5. Runtime 是否支持幂等执行。
...
| # | 决策 | 内容 | 修订位置 |
|---|---|---|---|
| D1 | 提案 1：同步提交通道 | 废弃 outbox 轮询，Gateway 同步调会话管理 submit 拿回执，回执幂等落账 turn_admission | 7.1.1 / 7.2 / 7.3 |
| D2 | committed_content 降级 | 权威移至会话管理消息表；Gateway 侧 committed_content 仅作本地镜像/缓存（可重建） | 5.1.5 / 9.3 |
| D3 | 快照正文归属会话管理 | 快照正文权威在会话管理（snapshot 接口存储）；invocation_spec 只留 snapshot_id + content_hash | 5.1.3 / 8.2 / 8.4 |
| D4 | Phase 2~7 不变 | 调度流水线核心（排队/租约/下拨/续读）维持既有设计 | 6 |
| D5 | 状态机走新方案 | Turn PENDING 起点（非 QUEUED）、终态 COMPLETED；Attempt PENDING→RUNNING 由首次 commit 隐式触发；Attempt 终态 COMPLETED（非 SUCCESS） | 3.2 / 5.2 / 5.3 |
| D6 | 三段映射 | 前端 API→Gateway→会话管理契约逐条映射（submit/retry/cancel/commit/update） | 7.1.1 / 12.1 |
| D7 | Attempt 归会话管理 | Attempt 业务记录只由会话管理创建（submit 建 #1，retry 建 #N）；Gateway 消费 attempt_id 建 Invocation，不直接 INSERT attempt | 7.1.1 |
| D8 | 内部接口围绕外部契约 | 4.5 接口按 A1~A12 契约重写（SessionManagementClient 签名） | 4.5 |
| D9 | 异常默认自动重试 | 所有异常默认自动重试（INFRA_RETRY），非"v1 一律中断" | 10.2 / 10.3 |
| D10 | Commit 分界 | 未 commit → 同一 Attempt 内重派 N=5 退避；已 commit → Attempt 判 failed，Turn 自动 INFRA_RETRY 新 Attempt（复用快照） | 10.2 / 9.1 |
| D11 | 停止与恢复 | 自动重试停止 = 无可用 Runtime（挂起）/ 用户手动停止（永久挂起，仅 USER_RETRY）；恢复 = 巡检唤醒 | 10.2 |
| D12 | 挂起落点 | 未 commit 挂 invocation `paused`（resume 续跑）；已 commit 失败后新 Attempt 挂 admission `suspended`（唤醒后重新竞争） | 11.3 / 5.1.1 / 5.1.2 |
| D13 | 唤醒不新增组件 | 复用 RecoveryCoordinator 巡检兼任唤醒器，逐个唤醒最新 Turn 的最新 Attempt | 11.3 / 11.8 |
| D14 | 直连 Pod IP 寻址 | Nacos 元数据带 ip:port（K8s Pod IP；本地 debug 127.0.0.1）；holder_pod/runtime_replica_id 为身份标识非 endpoint | 2.4 / 9.1 / 5.3.2 |
| D15 | 幂等键调整 | turn_admission 删 client_app_id/request_id；幂等键 = (session_id, turn_id, attempt_id) 回执；用户防重放前移 Gateway 对外 API | 5.1.1 / 7.3 / 7.4 |
| D16 | 容量测算局部修订 | §14 删 outbox 测算，补同步 RPC + INFRA_RETRY 写放大（重试率 ~5%） | 14.3 |

> **本批决策 D17~D27（采纳《architecture-review.md》审阅建议 R1~R11 及 C3，编号延续）**：逐条拍板，标注修订位置。

| # | 决策 | 内容 | 修订位置 |
|---|---|---|---|
| D17 | 终态回写异步化 | Phase 7 本地事务只落终态 + 释放租约，COMMIT 后再异步调会话管理 `update`；巡检判据 4 补发终态直至对齐（消除 RPC 夹在事务中的双写窗口） | 10.1 / 11.5 / 11.2 / 15.1 |
| D18 | commit fencing 校验 | commit 携带 attempt_id + controller_epoch，Gateway 与会话管理双侧校验（活跃 attempt + 当前 holder），旧 holder 残留写入一律拒绝 | 9.2 / 7.1.1 / 5.1.5 |
| D19 | deadline 强制超时 | 巡检判据 5：`executing` 且超 spec.deadline → 按 commit 分界走自动重试（10.2），杜绝死循环/无限卡死 | 11.5 / 8.5 |
| D20 | 人工干预接口 | admin 提优先级 / 强制 failed / 唤醒重排，全部落 `admission_audit` 审计轨迹；干预须走状态机合法迁移 | 11.7 / 5.7 |
| D21 | 重试配额隔离 | `infra_retry_count` 独立计数（上限 3），与 USER_RETRY 配额互不消耗——基础设施故障不得耗尽用户重试机会 | 7.1.1 / 10.3 |
| D22 | Runtime 事件拉模型 | 事件改为 Gateway 主动长连/轮询拉取（跟 holder 走），免 Runtime 端事件推送与发现逻辑 | 9.4 / 11.2 |
| D23 | fencing token 生命周期 | token 有效期 = Invocation 生命周期，代际 = controller_epoch（被接管即失效） | 6.3 |
| D24 | 快照孤儿回收 | Attempt 终态时会话管理侧引用计数递减回收正文；孤儿兜底 = 创建时间 + 无活跃引用周期扫描（TTL 24h） | 8.4 / 8.2 |
| D25 | L1 降级声明 | v1.0 不实现 K8s 节点级提前接管（L1），接管一律 L2 租约过期；L1 作为后续增强项 | 11.2 |
| D26 | 终态数据生命周期分层 | admission 冷热分层不物理删除（重放防护上移为"终态拒绝"，7.3）；invocation/spec 转历史表（审计）；committed_content 可删（镜像可重建）；lease_info 保持现状自清理 | 14.8 / 7.3 / 5.1.5 |
| D27 | 任务级证据 | F4 无可用 Runtime 判定 = Nacos 池空 + 连续派发失败（任务级证据），避免单点误判 | 12 / 11.3 |
| D28 | 取消语义正交化 | 停止语义按**触发者**正交：INTERRUPTED 态全层删除（v1 残留）；用户取消四层同名 CANCELLED（Turn/Attempt 经 SM.cancel 事务内同落，Admission/Invocation 由 Gateway 联动）；update 枚举收敛为 COMPLETED/FAILED；系统故障一律 FAILED + fail_reason（副作用未知 = SIDE_EFFECT_UNKNOWN，不自动重试）；cancel_reason 仅'用户取消'；CANCELLED 可 USER_RETRY 恢复；排队/挂起中均可取消；Runtime 提供 abort 接口（fencing 校验 + lease 到期兜底）；挂起无自动超时（仅用户 cancel / admin forceFail 终态化） | 5.2~5.7 / 7.1.1 / 5.1.1 / 9.1 / 11.4 / 10.2 / 10.3 / 4 / 7~12 |
